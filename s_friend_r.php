...
<link href="css/s_friend_r.css" rel="stylesheet" type="text/css">
<style type="text/css">
</style></head>
<script>
</script>
...
$txtsearch=addslashes( $_REQUEST["txtsearch"]);
...
?>  
  	<div id="searchfrndbox">
  <div id="srcfrndphoto"></div>
  <div id="srcfrndname">
 <?php echo $list0search['fname']." ".$list0search['lname'];?> 
  </div>
  <div id="srcfrndstatus">
 <?php 
   $query2search = "SELECT * FROM `friends` WHERE `sender`='$username'  AND `receiver`='$addusername'";
   $result2search = mysql_query($query2search, $conn);
   if(mysql_num_rows($result2search) !== 0){
	   	$query3search = "SELECT * FROM `friends` WHERE `sender`='$username'  AND `receiver`='$addusername' AND `accept`='no'";
   		$result3search = mysql_query($query3search, $conn);
   		if(mysql_num_rows($result3search) !== 0){
	   	echo 'friend request sent';}	
   				
		$query4search = "SELECT * FROM `friends` WHERE `sender`='$username' AND `receiver`='$addusername' AND `accept`='yes'";
  		$result4search = mysql_query($query4search, $conn);
  		if(mysql_num_rows($result4search) !== 0){
	   	echo 'Friend Request Accepted';}
   	}
	$query5search = "SELECT * FROM `friends` WHERE `sender`='$addusername'  AND `receiver`='$username'";
    $result5search = mysql_query($query5search, $conn);
	if(mysql_num_rows($result5search) !== 0){
	   	$query6search = "SELECT * FROM `friends` WHERE `sender`='$addusername'  AND `receiver`='$username' AND `accept`='no'";
   		$result6search = mysql_query($query6search, $conn);
   		if(mysql_num_rows($result6search) !== 0){
  ?>
	   <a href="accept_req.php?fname=<?php echo $list0search['fname'];?>&lname=<?php echo $list0search['lname'];?>">
       			 + Accept Request + </a>
  <?php	
   	}	
   		$query7search = "SELECT * FROM `friends` WHERE `sender`='$addusername' AND `receiver`='$username' AND `accept`='yes'";
  		$result7search = mysql_query($query7search, $conn);
  		if(mysql_num_rows($result7search) !== 0){
	   	echo 'Friend Request Accepted';}
   	}					
	if(mysql_num_rows($result2search) === 0 && mysql_num_rows($result5search) === 0) {
 ?> 
    <a href="send_req.php?fname=<?php echo $list0search['fname'];?>&lname=<?php echo $list0search['lname'];?>">
     			+ Add Friend + </a> 
 <?php } ?>  
  </div>
  	</div>
...
<div id="frndhead" align="center"> Friends </div>
	<div id="pnlfrnd">
